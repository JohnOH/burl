...
if (( $now > $ts+59 )); then	# Max one sample per minute
	...
	if [ "$lastAvg" -ne "$Avg" ]; then
		...
		if [ "$Avg" -gt "17" ]; then
			...
			/usr/bin/jee 17,170p	# 54.0° Reasonable 4/1/19
		elif [ "$Avg" -eq "11" ]; then
			/usr/bin/jee 17,176p	# 55.2° Reasonable 4/1/19
		elif [ "$Avg" -eq "10" ]; then
			/usr/bin/jee 17,177p	# 55.4°	Reasonable 23/1/19
		elif [ "$Avg" -eq "9" ]; then
			/usr/bin/jee 17,181p	# 56.2°
