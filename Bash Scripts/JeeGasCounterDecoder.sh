...
let "temperature = $temperature-9"    # Calibration

#echo "Temperature at gas meter:$temperature"
...
if [ "${yymmdd:6:2}" -ne "${date:8:2}" ] 
  then
#  echo JeeGasCounterDecoder.sh >&2
  ...
  /bin/cat /etc/heyu/jeebash.err >> /tmp/email
  mv /etc/heyu/jeebash.err /etc/heyu/archive/jeebash-${date:0:4}${date:5:2}${date:8:2}.err
  touch /etc/heyu/jeebash.err
  echo '------------burl.err------------' >> /tmp/email
  /bin/cat /etc/heyu/burl.err >> /tmp/email
  mv /etc/heyu/burl.err /etc/heyu/archive/burl-${date:0:4}${date:5:2}${date:8:2}.err
  touch /etc/heyu/burl.err  
  echo '------------post.err------------' >> /tmp/email
  /bin/cat /etc/heyu/post.err >> /tmp/email
  mv /etc/heyu/post.err /etc/heyu/archive/post-${date:0:4}${date:5:2}${date:8:2}.err
  touch /etc/heyu/post.err  
  echo '------------receive.log------------' >> /tmp/email
  touch /tmp/receive.log
...
Date: `date`
Subject: Sandhurst Energy Management: JeeGasCounterDecoder `date -R`
Body:
------------------------
`/bin/cat /tmp/email`
------------------------
